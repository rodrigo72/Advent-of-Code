...
public class Game {
    ...
    private final List<List<Subset>> subsets;
    private static final Map<Color, Integer> colorQuantity = new HashMap<>();
    ...
    public Map<Color, Integer> getMinimumSet() {
        ...
        for (List<Subset> subsets : this.subsets)
            for (Subset subset : subsets)
                maxQuantity.compute(subset.color, (color, currentMax) ->
                        currentMax == null || subset.quantity > currentMax ? subset.quantity : currentMax);
        return maxQuantity;
    }
